...
# 금지 규칙에 해당되면 반드시 is_hr_question을 false로 판별하세요
# =========================
# 1차 라우터: HR Router
# =========================
...
def hr_router(state: State) -> State:
    """
    먼저 금지 규칙을 판별하고, 그 후 HR 여부를 판별하여 상태에 저장
    """
    prompt = f"""
    당신은 "가이다 플레이 스튜디오(GPS)"의 HR 안내 챗봇입니다.
    아래의 정제 질문을 보고, 먼저 금지 규칙에 해당하는지 확인한 후 HR 관련 여부를 판별하세요. 

    정제 질문: "{state['refined_question']}"

    판별 규칙 (우선순위 반드시 지켜야 함):

    1. 금지 규칙 확인
    정제 질문이 아래 금지 규칙에 해당한다면, 무조건 다음과 같이 출력해야 합니다:
       {{"is_hr_question": false}}

    금지 규칙:
     - 개인과 직접적으로 연결된 HR 데이터는 금지
       (예: "내 월급 안내", "내 퇴직금 계산")
     - 개인정보 (예: 주민등록번호, 이름)
     - 회사 내부 보안 내용 (예: 회사 재정 상황, 신규 프로젝트, 회사의 중요한 내부 문건)
     - 복잡한 급여·퇴직금 계산 요청(예: "퇴직금 계산 안내", "급여 계산 안내")
     - 법률 자문 요청이나 법률 상담 톤의 질문

     반대로, 개인을 특정하지 않는 일반 정책이나 제도 관련 질문은 금지가 아닙니다.
     (예: "급여일이 언제야?", "퇴직금 안내")

    2. 금지 규칙이 아닌 경우에만 HR 여부를 판별하세요:
