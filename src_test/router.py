...
def _classify_rag_or_department(state: State) -> Dict[str, str]:
    """
    ...
    * "장비 사용 규칙이 궁금해요"
    ...
    _llm_model = ChatOpenAI(model="gpt-4.1-mini", temperature=0)
...
def route_question(state: State) -> State:
    ...
    # LLM을 통한 통합 분류
    _llm_model = ChatOpenAI(model="gpt-4.1-mini", temperature=0)
    classification_result = _llm_model.classify_with_llm(question)
...
def generate_department_response(state: State) -> str:
    """
    담당자 안내 응답 생성
    """
...
def should_use_rag(state: State) -> bool:
    """
    RAG 사용 여부 판단
    """
    return state.get('is_rag_suitable', False) 

def process_secondary_routing(state: State) -> State:
    """
    2차 라우터 전체 처리 프로세스
    """
    # 1. 질문 분류 및 라우팅
    ...
    if state['answer_type'] == "department_contact":
        state['final_answer'] = generate_department_response(state)
